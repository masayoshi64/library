...
data:
  ...
  bundledCode: "#line 1 \"verify/aoj-DPL_5_G.test.cpp\"\n#define PROBLEM \\\n    \"\
    ...
    #line 1 \"library/math/combination.cpp\"\n/**\n * @brief Combination(P, C, H,\
    \ Stirling number, Bell number)\n * @docs docs/Combination.md\n */\ntemplate <typename\
    \ T>\nstruct Combination {\n    vector<T> _fact, _rfact, _inv;\n\n    Combination(int\
    \ sz) : _fact(sz + 1), _rfact(sz + 1), _inv(sz + 1) {\n        _fact[0] = _rfact[sz]\
    \ = _inv[0] = 1;\n        for (int i = 1; i <= sz; i++) _fact[i] = _fact[i - 1]\
    \ * i;\n        _rfact[sz] /= _fact[sz];\n        for (int i = sz - 1; i >= 0;\
    \ i--) _rfact[i] = _rfact[i + 1] * (i + 1);\n        for (int i = 1; i <= sz;\
    \ i++) _inv[i] = _rfact[i] * _fact[i - 1];\n    }\n\n    inline T fact(int k)\
    \ const { return _fact[k]; }\n\n    inline T rfact(int k) const { return _rfact[k];\
    ...
    \u30B0\u30EB\u30FC\u30D7\u306B\u5206\u3051\u308B\u3068\u8003\u3048\u3066\u3082\
    \u3044\u3044\n    T Bell(int n, int k) {\n        if (n < k) k = n;\n        vector<T>\
    \ sm(k + 1);\n        sm[0] = 1;\n        rep(j, 1, k + 1) { sm[j] = sm[j - 1]\
    \ + (T)(j % 2 ? -1 : 1) / _fact[j]; }\n        T res = 0;\n        rep(i, k +\
    \ 1) { res += mypow<T>(i, n) / _fact[i] * sm[k - i]; }\n        return res;\n\
